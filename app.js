                    ...
                    if (err) console.log(err);
                        ...
                        for (var i = 0; i < 100; i++) {
                            ...
                            if (result[0].number == number) {
                                ...
                                mysql.query('INSERT INTO news (number,title,content,date) VALUES (?,?,?,?)', [number, title, content, date], function(err, result) {
                                    if (err) console.log(err);
                                    else {}
                                });
                            }
...
app.get('/sendData',function(req,res){
    mysql.query('SELECT * FROM news ORDER BY number DESC LIMIT 100',function(err,result){
        if(err) console.log(err);
        else{
